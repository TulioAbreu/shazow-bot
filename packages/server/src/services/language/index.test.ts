...
describe("Language", () => {
    ...
    test("should parse output with argument", () => {
        const output = getOutput(Output.AnimeFetchFailed, Language.PortugueseBrazil, [
            "animeError"
        ]);
        expect(output).toBe("ocorreu algum problema enquanto procurava seu anime. Tente mais tarde! Erro: animeError");
    });
});
